...
class GraspNode(Node):
	...
	def compute_euler(self,pose_T):
		P1 = np.array([self.CurEndPos[0], self.CurEndPos[1]])  # 点 P1 的坐标
		P2 = np.array([pose_T[0], pose_T[1]])  # 点 P2 的坐标
		# 计算两点之间的欧几里得距离
		distance_xy = np.linalg.norm(P2 - P1) 
		print("distance_xyz: ",distance_xy)

		# 计算z之间的欧几里得距离
		distance_z = self.CurEndPos[0] - pose_T[2]
		print("distance_xy: ",distance_z) 
    
		self.angle_radians =  np.arctan(abs(distance_z)/abs(distance_xy))
        
		if distance_z>0:
			self.angle_radians = 1.57 - self.angle_radians
	...
	def grasp(self,pose_T):
		...
		request.tar_x = pose_T[0]   + 0.03
		request.tar_y = pose_T[1] 
		request.tar_z = pose_T[2]  
		#print("request.tar_z: ",request.tar_z)
		request.kin_name = "ik"
		request.roll = self.angle_radians #3.14
		request.pitch  =  self.CurEndPos[4]  #1.919  self.angle_radians#self.angle_radians#self.angle_radians
		request.yaw = self.CurEndPos[5]#1.57
		#print("calcutelate_request: ",request)
		print("dist: ",math.sqrt(request.tar_x ** 2 + request.tar_y** 2)*1000)
	...
	def move(self):
		self.pubSingleArm(6, self.joint6, 2000)#机械臂夹紧（夹取目标）
		time.sleep(2.5)
		self.pubSixArm([90, 180, 0, 0, 90,self.joint6])	
		# time.sleep(2.5)
		# if self.cur_tagId == 1:
	...
	def pubSixArm(self, joints, id=6, angle=180.0, runtime=2000):
		...
		arm_joints.joint1 = int(180 - joints[0])
