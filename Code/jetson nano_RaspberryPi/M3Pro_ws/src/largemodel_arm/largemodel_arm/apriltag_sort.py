...
from std_msgs.msg import Float32,Bool,UInt16,Int16,String
...
class AprilTagDetectNode(Node):
	...
	def get_graspStatusCallBack(self,msg):
		...
		self.adjust_dist = True
		self.detect_flag = False
		sort_done_single = String(data='apriltag_sort_done')
		self.largemodel_arm_done_pub.publish(sort_done_single)
		...
		if len(tags) > 0 :
			for i in range(len(tags)):
				...
				if cur_id == self.TargetID  and self.pubPos_flag  == True:
					...
					self.index = i
					if abs(dist_detect - 200.0)>10 and self.adjust_dist==True and self.detect_flag == True:
						self.move_dist(dist_detect)
					elif abs(dist_detect - 200.0)<10 and self.detect_flag == True:
						self.pubVel(0,0,0)
						self.adjust_dist = False
						tag = AprilTagInfo()
						tag.id = tags[self.index].tag_id
						tag.x = self.Center_x_list[self.index]
						tag.y = self.Center_y_list[self.index]
			...
			if 	self.detect_flag == False and self.TargetID!=0 and self.pubPos_flag==True:#没找到机器码
				...
				self.get_logger().info('Did not find the target.')
				self.largemodel_arm_done_pub.publish(String(data='apriltag_sort_failed'))
				self.TargetID = 0
				reset = Bool()
				reset.data = True
