...
class KCFTrackNode(Node):
	def __init__(self, name):
		...
		self.init_joints = [90, 130, 0, 0, 90, 105]
	...
	def move_dist(self,dist):
		...
		if abs(dist - self.grasp_Dist) < 20: 
			linear_x = 0
			self.adjust_dist = False
			self.start_grasp = True
		self.pubVel(linear_x*0.6,0,0)
	...
	def callback(self,msg):
		...
		if cur_depth!=999.0:  
			...
			# if (abs(center_x-320) >20 or abs(center_y-240)>20) and self.XY_Track_flag==True:
			# 	self.XY_track(center_x,center_y)
			# 	print("Tracking")
			# 	print("-------------------------------------")

			# if abs(center_x-320) <20 and abs(center_y-240)<20 and self.Done_flag==True:
			# 	self.adjust_dist = True
			...
			if self.adjust_dist== True:
				if abs(self.cur_distance - 260.0)>10:
