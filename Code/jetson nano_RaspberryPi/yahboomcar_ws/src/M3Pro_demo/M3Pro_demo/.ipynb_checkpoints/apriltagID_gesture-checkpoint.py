	...
	def callback(self,color_msg,depth_msg):
		...
		if len(tags) > 0 :
			for i in range(len(tags)):
				...
				if abs(dist_detect - 215.0)>5 and self.adjust_dist==True and self.detect_flag == True and self.index!=None:
					print("Adjusting")
					self.move_dist(dist_detect)
				elif abs(dist_detect - 215.0)<5 and self.detect_flag == True and self.index!=None:
					self.pubVel(0,0,0)
