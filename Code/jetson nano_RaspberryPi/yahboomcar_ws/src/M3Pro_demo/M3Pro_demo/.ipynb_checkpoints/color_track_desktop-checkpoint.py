...
import threading
from X5Plus_demo.compute_joint5 import *
...
class ColorRecognizeNode(Node):
	...
	def callback(self,color_frame,depth_frame):
		...
		if self.cx!=0 and self.cy!=0 and self.circle_r>20:
			#print("self.cx: ",self.cx)
			#print("self.cx: ",self.cy)
			cx = int(self.cx)
			cy = int(self.cy)
			if (abs(cx-320) >10 or abs(cy-300)>10) and self.move_flag == True: 
				print("Tracking now.")
				print("-------------------")
			...
			elif abs(cx-320) <10 and abs(cy-300)<10 and self.pub_pos_flag == False and self.done_flag==True:
				self.pubVel(0,0,0) 
