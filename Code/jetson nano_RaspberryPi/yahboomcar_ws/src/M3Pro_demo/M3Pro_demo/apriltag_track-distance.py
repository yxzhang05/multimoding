...
class AprilTagTrackNode(Node):
	def __init__(self, name):
		...
		self.lineary_PID = (0.2, 0.0, 0.08)
		self.angz_PID = (50.0, 0.0, 20.0)
	...
	def robot_move(self,point_x,dist):
		...
		if abs(self.prev_angular - point_x) > 300:
			...
			return
		linear_x = self.linearx_pid.compute(dist, self.minDist)*0.8
		linear_y = self.lineary_pid.compute(320, point_x)*0.5
		#angular_z = self.angz_PID.compute(320, (180 - abs(roll_degree)) / 36, 0)
		if abs(dist - self.minDist) < 15: linear_x = 0.0
		if abs(point_x - 320.0) < 5: linear_y = 0.0
		self.pubVel(linear_x,linear_y,0)

	def robot_rotation(self,roll):
		roll_degree = abs(math.degrees(roll))
		if abs(self.prev_roll - roll_degree) > 10:
			self.prev_roll = roll_degree
			print("+++++++++++++++++++++++++")
			return
		angular_z = self.angz_pid.compute((175 - abs(roll_degree)) / 72, 0)
		if abs(roll_degree - 175) < 18: 
			angular_z = 0.0
			self.roll_track_flag = False
			print("**********************")
			print("Rotation done.")
		#print("angular_z: ",angular_z)
		self.pubVel(0,0,angular_z)
	...
	def pubVel(self,vx,vy,vz):
		...
		vel.angular.z = self.rotation_direction*float(vz)
	...
	def callback(self,color_frame,depth_frame):
		...
		if len(tags) > 0 :
			...
			self.cur_distance = depth_image_info[int(center_y),int(center_x)]
			...
			if (3.04 - euler_angles[0])<0:
				self.rotation_direction = -1
			if (abs(center_x-320) >5 or abs(self.cur_distance - self.minDist) > 15) and self.xy_track_flag == True:
				print("Translation.")
				self.robot_move(center_x,self.cur_distance)
			elif abs(center_x-320) <5 and abs(self.cur_distance - self.minDist)<15 and self.xy_track_flag == True:
				print("Translation done.")
				self.xy_track_flag = False
				self.pubVel(0.0,0.0,0.0)
			if self.xy_track_flag==False and self.roll_track_flag==True: 
				print("Rotation.")
				self.robot_rotation(euler_angles[0])   

			if self.roll_track_flag == False and abs(center_x-320) >5 and self.move_y == True:
				print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
				self.robot_move(center_x,self.minDist)
			elif abs(center_x-320) <5 and self.roll_track_flag == False and self.move_y == True:
				self.pubVel(0.0,0.0,0.0)
				print("Adjust was done,start to grasp.")
				self.move_y = False
