...
package_path= "/home/jetson/yahboomcar_ws/src/M3Pro_demo/M3Pro_demo"
...
class ColorRecognizeNode(Node):
	def __init__(self, name):
		...
		self.at_detector = Detector(searchpath=['apriltags'], 
                                    ...
                                    debug=0)
		self.CurEndPos =[0.13850614623645266, 0.000231128779644737, 0.16629191805489946, 0.00018255962454541956, 1.221730317129278, 0.00014750319860679475]
		...
		self.target_color = 0
		self.red_hsv_text = os.path.join(package_path,'red_colorHSV.text')  
		self.green_hsv_text = os.path.join(package_path,'green_colorHSV.text')
		self.blue_hsv_text = os.path.join(package_path,'blue_colorHSV.text')
		self.yellow_hsv_text = os.path.join(package_path,'yellow_colorHSV.text')
		self.hsv_range = ()
		...
		self.target_high = self.get_parameter('target_high').get_parameter_value().double_value * 100
		...
		if len(self.CX_list)>0 and self.done_flag==True:
			for i in range(len(self.CX_list)):
				if self.R_list_[i]>30:
					...
					print(heigh)
					cv.putText(result_image, heigh, (int(cx)+10, int(cy)-25), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
					if  compute_heigh > self.target_high and self.pub_pos_flag == True and self.R_list_[i]>30:
						self.index = i
						self.detect_flag = True
						self.compute_height = False
					if self.detect_flag == True and self.index!=None:
						cx = self.CX_list[self.index]
						cy = self.CY_list[self.index]
						...
						self.get_logger().info(dist)
						cv.putText(result_image, dist, (int(cx)+5, int(cy)+15), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
						if self.detect_flag == True and self.index != None and self.valid_dist==True and self.R_list_[self.index]>30:
					
							if abs(dist_detect - self.grasp_Dist)>5 and self.adjust_dist==True:
