		...
		if len(tags) > 0 :
			for i in range(len(tags)):
				...
				if cur_id == self.TargetID  and self.pubPos_flag  == True:
					...
					self.index = i
					if abs(dist_detect - self.grasp_Dist)>10 and self.adjust_dist==True and self.detect_flag == True:
						self.move_dist(dist_detect)
					elif abs(dist_detect - self.grasp_Dist)<10 and self.detect_flag == True:
						self.pubVel(0,0,0)
