...
class laserAvoid(Node):
    ...
    def registerScan(self, scan_data):
        ...
        for i in range(len(ranges)):
            ...
            #30 130
            if (self.LaserAngle*0.5 > abs(angle) > self.LaserAngle) and ranges[i] !=0.0 :
                if ranges[i] < self.ResponseDist*1.5:
                    self.Right_warning += 1
            if (360 - self.LaserAngle < abs(angle) <  360 - self.LaserAngle + self.LaserAngle*0.5) and ranges[i] !=0.0 :
                if ranges[i] < self.ResponseDist*1.5:
                    self.Left_warning += 1
