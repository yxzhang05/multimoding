...
class AprilTagDetectNode(Node):
	...
	def get_GesturetIdCallBack(self,msg):
		self.Target_dist = msg.data*10 + self.Target_dist
		self.pubSix_Arm(self.init_joints)
	...
	def callback(self,color_msg,depth_msg):
		...
		Target_dist = "Target_dist: " + str(self.Target_dist) + "mm"
		cv.putText(rgb_image, Target_dist, (25, 25), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
		key = cv2.waitKey(10)
		if key == 32:
			self.pubPos_flag = True
			self.pubSix_Arm(self.init_joints)
		...
		if len(tags) > 0 and self.done_flag == True:
			for i in range(len(tags)):
				...
				compute_detect = math.sqrt(pose[1] ** 2 + pose[0]** 2)
				compute_detect = compute_detect*1000
				dist = 'dist: ' + str(compute_detect) + 'mm'
				cv.putText(rgb_image, dist, (int(cx)+5, int(cy)-15), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
				#cv.putText(rgb_image, dist, (int(cx)+5, int(cy)+15), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
				if compute_detect > self.Target_dist and self.pubPos_flag == True and self.compute_dist == True:
					print("Found the target.")
					print("compute_detect: ",compute_detect) 
				...
				if self.detect_flag == True and self.index != None:
					...
					if abs(dist_detect - 200.0)>10 and self.adjust_dist==True:
			...
			if 	self.detect_flag == False and self.Target_height!=220 and self.pubPos_flag == True:
				self.pubPos_flag = False
				self.Beep_Loop()
				self.shake()
