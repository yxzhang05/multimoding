...
class AprilTagDetectNode(Node):
	def __init__(self, name):
		super().__init__(name)
		self.init_joints = [90, 150, 0, 0, 90, 90]
	...
	def callback(self,color_frame,depth_frame):
		...
		if len(tags) > 0 :
			...
			if self.compute_flag==True:
				self.ctrl_joint5(target_joint5)
			if self.pubPos_flag == True and tag.z!=0.0:
				self.TargetJoint5_pub.publish(self.joint5)
				self.pos_info_pub.publish(tag)
				self.pubPos_flag = False
				self.compute_flag = False
			elif self.pubPos_flag == True and tag.z==0.0:
				print("Invalid Dist.")
		else:
			self.pubVel(0,0,0)
	...
	def ctrl_joint5(self,joint5_value):
		j1 = 90
		if joint5_value<0:
			j5 = abs(joint5_value)
			if abs(joint5_value)<90:
				j5 =180-j1+j5 -90
			...
			if j5 >135:
				...
				j5 =  j5 + 90
			self.pubSingleArm(5,int(j5),800)
			self.joint5.data = int(j5)
			self.TargetJoint5_pub.publish(self.joint5)

		if joint5_value>0:
			j5 = 180 - abs(joint5_value)
			if joint5_value<90:
				j5 = j5  - j1
			else :
				j5 = j5 - (j1 - 90)
