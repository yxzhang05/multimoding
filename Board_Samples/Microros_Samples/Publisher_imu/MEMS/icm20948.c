...
signed char COMPASS_ORIENTATION[]={ 0, -1, 0, 
                                    1, 0, 0, 
                                    0, 0, 1
                                    };
...
void process_sensor_output()
{
    ...
    if (quat6_data_was_set == 1)
    {
        if (hal.report & PRINT_GRV)
        {
            ...
            values[2] = atan2(2 * (q01 + q23), (1 - 2 * (q11 + q22)));   // Yaw
...
//					long accelQ16[3];
        ...
        if (header & PED_STEPDET_SET)
        {
            if (hal.report & PRINT_STEP_COUNTER)
            {
                ...
                if (steps != old_steps)
                {
                    if(usart_dbg)printf("\tStep Counter %ld\r\n", steps);
...
int ICM_20948_Init(void)
{
    ...
    tx = 0x80;
    result |= inv_serial_interface_write_hook(0x06, 1, &tx);
    osDelay(100);
    tx = 0x00;
    result |= inv_serial_interface_write_hook(0x7f, 1, &tx);
    ...
    if (usart_dbg)
    {
        if (result)
        {
            printf("ICM20948 Could not initialize.\r\n");
        }
        else
        {
            printf("ICM20948 Initialize Sucessful...\r\n");
        }
    }
...
{
    return g_icm_data.orientation[0];
    // return g_icm_data.orientation[0]*RtA;
}

float ICM20948_Get_Pitch_Now(void)
{
    return g_icm_data.orientation[1];
    // return g_icm_data.orientation[1]*RtA;
}

float ICM20948_Get_Yaw_Now(void)
{
    return g_icm_data.orientation[2];
    // return g_icm_data.orientation[2]*RtA;
}
