...
{
  (uint8_t *)(void *)joystick_report_data + AXIS_LX, /*data*/
  8,     /*size*/
  0,     /*shift*/
...
{
  (uint8_t *)(void *)joystick_report_data + AXIS_LY, /*data*/
  8,     /*size*/
  0,     /*shift*/
  0,     /*count (only for array items)*/
  1,     /*signed?*/
...
{
  (uint8_t *)(void *)joystick_report_data + AXIS_RX, /*data*/
  8,     /*size*/
  0,     /*shift*/
  0,     /*count (only for array items)*/
  1,     /*signed?*/
...
{
  (uint8_t *)(void *)joystick_report_data + AXIS_RY, /*data*/
  8,     /*size*/
  0,     /*shift*/
  0,     /*count (only for array items)*/
  1,     /*signed?*/
...
{
  (uint8_t *)(void *)joystick_report_data + BUTTON_LEFT, /*data*/
  4,     /*size*/
  0,     /*shift*/
  0,     /*count (only for array items)*/
  0,     /*signed?*/
...
{
  (uint8_t *)(void *)joystick_report_data + BUTTON_RIGHT, /*data*/
  1,     /*size*/
  0,     /*shift*/
  0,     /*count (only for array items)*/
  0,     /*signed?*/
...
{
  (uint8_t *)(void *)joystick_report_data + BUTTON_MIDDLE, /*data*/
  1,     /*size*/
  5,     /*shift*/
  0,     /*count (only for array items)*/
  0,     /*signed?*/
...
static void print_raw_data(HID_HandleTypeDef *HID_Handle)
{
    printf("Joy Raw(HEX):");
...
static USBH_StatusTypeDef USBH_HID_JoystickDecode(USBH_HandleTypeDef *phost)
{
  ...
  if (USBH_HID_FifoRead(&HID_Handle->fifo, &joystick_report_data, HID_Handle->length) ==  HID_Handle->length)
  {
    ...
    if(!is_diff && ((HAL_GetTick() - prev_time) <  JOY_SEND_TIME_MS)) {
    	return USBH_OK;
    }
