...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  else if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol  == 0x00)
  {
	...
	// if((phost->device.DevDesc.idVendor == 0x2563 || phost->device.DevDesc.idVendor == 0x79 || phost->device.DevDesc.idVendor == 0x45e))
	if((phost->device.DevDesc.idVendor == 0x79 && phost->device.CfgDesc.Itf_Desc[interface].bInterfaceClass == 0x03))
	{
    ...
    {
      USBH_UsrLog("Protocol not supported.VID:0x%X, PID:0x%X", phost->device.DevDesc.idVendor, phost->device.DevDesc.idProduct);
      return USBH_FAIL;
    }
  }
  else
  {
    USBH_UsrLog("Device no found:%d", phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol);
    return USBH_FAIL;
  }
